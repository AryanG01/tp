...
public class EditCommandParser implements Parser<EditCommand> {
    ...
    public EditCommand parse(String args) throws ParseException {
        ...
        if (argMultimap.getValue(PREFIX_ADDRESS).isPresent()) {
            editRoomDescriptor.setAddress(ParserUtil.parseAddress(argMultimap.getValue(PREFIX_ADDRESS).get()));
        }
        if (argMultimap.getValue(PREFIX_START_DATE).isPresent()) {
            editRoomDescriptor.setStartDate(ParserUtil.parseBookingDate(argMultimap.getValue(PREFIX_START_DATE).get()));
        }
        if (argMultimap.getValue(PREFIX_END_DATE).isPresent()) {
            editRoomDescriptor.setStartDate(ParserUtil.parseBookingDate(argMultimap.getValue(PREFIX_END_DATE).get()));
        }
